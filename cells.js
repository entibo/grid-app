...
export function moveRangeBy(cells, range, offset, removeDestination = true) {
  function removeOverlappingCells(cells, range, offset) {
    const targetRange = Range.move(range, offset)
    const [removedCells, keptCells] = partition(
      cells,
      (cell) =>
        Range.contains(targetRange, cell.position) &&
        !Range.contains(range, cell.position),
    )
    return [keptCells, removedCells.map((cell) => [CELL_REMOVED, cell])]
  }

  function moveOnly(cells, range, offset) {
    const changes = []
    const cellsAfterMove = cells.map((cell) => {
      if (!Range.contains(range, cell.position)) return cell
      const movedCell = {
        ...cell,
        position: Point.add(cell.position, offset),
      }
      changes.push([CELL_POSITION_CHANGED, movedCell])
      return movedCell
    })
    return [cellsAfterMove, changes]
  }

  if (!removeDestination) return moveOnly(cells, range, offset)

  return mapChanges(
    //
    removeOverlappingCells(cells, range, offset),
    (cells) => moveOnly(cells, range, offset),
  )
}

export function nudgeRangeBy(cells, range, offset) {
  const displacement = (() => {
    if (offset.dx === -1) {
      return {
        range: {
